...
module.exports.pushData_api = async (req, res) => {
  ...
  if (result == 0) {
    ...
    console.log(res)
    const checking = check.checkAreaChange(req.body.lat, req.body.long,res.lat,res.long)
    if(checking ==1) {
        console.log("here i am =====   2")
       await axios.get(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${req.body.lat}&longitude=${req.body.long}&localityLanguage=en`
          )
          .then(async (response) => {
